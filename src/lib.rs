use std::{os::macos::raw::stat, path::is_separator};
...
fn glob_match_internal<'a>(glob: &str, path: &'a str) -> bool {
  ...
  while state.glob_index < glob.len() || state.path_index < path.len() {
        ...
        b'*' => {
          ...
          if is_globstar {
            ...
            if glob.len() == state.glob_index {
              ...
              state.globstar.glob_index = state.wildcard.glob_index;
              state.globstar.path_index = state.skip_dirname(path) as u32;
      ...
      && state.globstar.path_index != state.wildcard.path_index
    {
      ...
      if state.globstar.path_index > 0 && state.globstar.path_index < path.len() as u32 {
...
impl State {
  ...
  fn skip_dirname(&mut self, path: &[u8]) -> usize {
...
impl BraceStack {
  ...
  fn pop(&mut self, state: &State) -> State {
    ...
    let mut prev_state = self.stack[self.length as usize];

    prev_state.glob_index = state.glob_index;
    prev_state.path_index = self.longest_brace_match as usize;
